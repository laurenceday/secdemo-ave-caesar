...
import {IWildcatMarketLike} from "../../src/WildcatDelinquencyTrigger.sol";
...
/// @notice Wildcat market mock: settable timeDelinquent and grace period,
///         returned via a full MarketState struct matching the mirrored
///         interface (only timeDelinquent is read by the trigger).
contract MockWildcatMarket {
    uint32 internal _timeDelinquent;
    uint256 internal _delinquencyGracePeriod;

    function setTimeDelinquent(uint32 t) external {
        _timeDelinquent = t;
    }

    function setDelinquencyGracePeriod(uint256 g) external {
        _delinquencyGracePeriod = g;
    }

    function delinquencyGracePeriod() external view returns (uint256) {
        return _delinquencyGracePeriod;
    }

    function currentState() external view returns (IWildcatMarketLike.MarketState memory s) {
        s.timeDelinquent = _timeDelinquent;
        s.isDelinquent = _timeDelinquent > 0;
    }
}
